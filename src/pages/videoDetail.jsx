...
//     try {
//       setDownloading(true);
//       console.log(`http://localhost:3000/download?url=${videoUrl}`);
...
// Multiple useEffects with Similar Logic: There are several useEffect hooks performing similar logic, such as updating views, fetching video details, and checking liked status. We can consolidate these to improve readability and reduce redundancy.

// Inline Styling: Inline styling can make the code harder to maintain and less flexible. It's better to move styles to CSS files or use a CSS-in-JS solution for better organization and reusability.

// Hardcoded URLs: Hardcoding URLs like "http://localhost:3000/download?url=${videoUrl}" makes the code less portable. It's better to store such URLs in environment variables or configuration files.

// Conditional Rendering Logic: The conditional rendering logic can be simplified and made more readable.

// Error Handling: There's limited error handling in the code. We can improve error messages and handle edge cases more gracefully.
...
function formatPublishedDate(publishDate) {
...
function VideoDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setVideoDetail({ error: "Video Doesn't exist" });
      }
    };
    fetchData();
  }, [id]);
  ...
  const onDownload = async (e) => {
    ...
    try {
      setDownloading(true);
      const response = await fetch(
        `${process.env.REACT_APP_DOWNLOAD_URL}?url=${videoUrl}`
      );
      if (response.ok) {
        console.log("Downloading video...");
        return response;
